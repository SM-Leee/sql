...
select concat(first_name,' ',last_name) from employees where emp_no like 10944;
...
select count(distinct emp_no) as '현재 근무하고있는 직원의 수' from salaries;
...
select count(distinct dept_no) as '부서의 매니저' from dept_manager;
...
select count(emp_no) from titles where title like 'engineer';
